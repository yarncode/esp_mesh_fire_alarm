...
static esp_netif_t *netif_eth = NULL;
    ...
    while (receive_task_is_running)
    {
        ...
        else
        {
            ...
            else if (data.proto == MESH_PROTO_STA)
            {
                ...
                {
                    // actual receive to TCP/IP stack
                    esp_netif_receive(netif_eth, data.data, data.size, NULL);
                }
            }
        }
    }
...
esp_netif_t *mesh_netifs_eth_init(mesh_raw_recv_cb_t *cb)
{
    ...
    netif_eth = esp_netif_new(&cfg_eth);
...
static esp_err_t start_wifi_link_sta(void)
{
    ...
    esp_err_t ret;
    void *driver = esp_netif_get_io_driver(netif_eth);
    if ((ret = esp_wifi_register_if_rxcb(driver, esp_netif_receive, netif_eth)) != ESP_OK)
    {
        ESP_LOGE(TAG, "esp_wifi_register_if_rxcb for if=%p failed with %d", driver, ret);
        return ESP_FAIL;
    }
    esp_netif_set_mac(netif_eth, mac);
    esp_netif_action_start(netif_eth, NULL, 0, NULL);
    return ESP_OK;
}
...
static esp_err_t start_mesh_link_sta(void)
{
    ...
    esp_netif_set_mac(netif_eth, mac);
    esp_netif_action_start(netif_eth, NULL, 0, NULL);
    esp_netif_action_connected(netif_eth, NULL, 0, NULL);
    return ESP_OK;
}
...
{
    ESP_LOGI(TAG, "Creating mesh link ap");
 ...
 */
static void destory_mesh_eth_link_ap(void)
{
    if (netif_ap)
    {
        ESP_LOGW(TAG, "Destroying mesh link ap");
...
esp_err_t mesh_netif_eth_start_root_ap(bool is_root, uint32_t addr)
{
    if (is_root)
    {
        ESP_LOGI(TAG, "Creating mesh link eth with ap");

        destory_mesh_eth_link_ap();
...
esp_err_t mesh_netifs_eth_start(bool is_root)
{
    if (is_root)
    {
        ...
        // Root: Station
        if (netif_eth && strcmp(esp_netif_get_desc(netif_eth), "eth") == 0)
        {
            ESP_LOGI(TAG, "Already eth station, no need to do anything");
        }
...
#if CONFIG_MESH_USE_GLOBAL_DNS_IP
        mesh_netif_eth_start_root_ap(true, htonl(DNS_IP_ADDR));
#endif
    }
...
{
    // if (netif_eth && strcmp(esp_netif_get_desc(netif_eth), "sta") == 0 && netif_ap == NULL)
    // {
    ...
    if (netif_eth)
    {
        if (strcmp(esp_netif_get_desc(netif_eth), "sta") == 0)
        {
            ESP_LOGW(TAG, "Already wifi station, no need to do anything");
            esp_netif_action_disconnected(netif_eth, NULL, 0, NULL);
            esp_netif_action_stop(netif_eth, NULL, 0, NULL);
            esp_wifi_clear_default_wifi_driver_and_handlers(netif_eth);
        }
        else
        {
            ESP_LOGW(TAG, "Already mesh link station, no need to do anything");
            esp_netif_action_disconnected(netif_eth, NULL, 0, NULL);
            mesh_delete_if_driver(esp_netif_get_io_driver(netif_eth));
        }
        ESP_LOGW(TAG, "Destroying wifi station");
        esp_netif_destroy(netif_eth);
        netif_eth = NULL;
    }
...
{
    mesh_netif_driver_t mesh = esp_netif_get_io_driver(netif_eth);
